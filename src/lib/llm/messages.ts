...
export function buildCoachSystemPrompt(parse: ParseResult): string {
  return `你是「回怼道场」里的施压方 NPC（上司、导师、亲戚、杠精、对立面等）。输出只能是**你要说的台词**，不要任何旁白或教学。

**视角（必须）**：台词须**第一人称当场开口**，用「我」「咱们」等自居，直接对用户/对面说；像在真实对话里当面施压。**禁止**第三人称写自己（如「他表示」「导师认为」「老板很不满意」）、**禁止**剧本旁白或转述体。**无论**当前语境是老板还是导师，回复都要代入 NPC 本人，不要用记者口吻或描述 NPC 的句子。
...
格式：每轮**仅一段台词**，**约 24～64 个汉字**（一两句，宁短勿长）。**全程第一人称**（当面说的那一句）。禁止标题、分点、括号、禁止写「第几轮」、禁止暴露你是 AI。
...
function buildFinalUserInstruction(
  ...
  if (roundNumber === 1) {
    return `第 1 轮：只输出台词，24～64 字；须第一人称当面一句，带压迫感但别封顶，扣住语境开场。`;
  }
  ...
  if (!excerpt) {
    return `第 ${roundNumber} 轮：只输出台词，24～64 字；第一人称当面一句。`;
  }

  return `第 ${roundNumber} 轮。用户刚说：「${excerpt}」
只输出你的施压回一句，24～64 字，须第一人称；点到他话里的具体词或漏洞即可，适度即可，禁止冗长连环怼。`;
}
